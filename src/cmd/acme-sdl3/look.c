/*
 * Modified acme/look.c for standalone acme with embedded plumber.
 * Changes from original:
 *   - startplumbing() loads plumber rules instead of connecting to external plumber
 *   - plumbthread() removed (no external plumber daemon)
 *   - look3() uses embedded plumb_send() for rule matching instead of plumbsendtofid()
 *   - plumblook/plumbshow called directly for "to edit" matches
 */
...
startplumbing(void)
{
	...
	if(plumb_init() < 0)
		warning(nil, "plumber: failed to load rules\n");
	else
		plumbing_ready = 1;
}
...
look3(Text *t, uint q0, uint q1, int external, int reverse)
{
	...
	if(plumbing_ready){
		/* send whitespace-delimited word to embedded plumber */
		...
		if(m->ndata<messagesize-1024){
			plumbresult = plumb_send(m);
			if(plumbresult == 0){
				/* matched "to edit" -- deliver directly */
				plumblook(m);
				plumbfree(m);
				goto Return;
			}
			if(plumbresult == 1){
				/* matched with start action -- already executed */
				plumbfree(m);
				goto Return;
			}
		}

/*
 * Embedded plumber: rule matching engine.
 * Adapted from src/cmd/plumb/match.c with renamed symbols.
 */
...
absolute(char *dir, char *file)
{
	...
	if(file[0] == '/')
		return plumb_estrdup(file);
	p = plumb_emalloc(strlen(dir)+1+strlen(file)+1);
...
verbisfile(int obj, Plumbmsg *m, Rule *r, Exec *e, ulong maskon, ulong maskoff, char **var)
{
	...
	default:
		...
		break;
	case OArg:
		file = absolute(m->wdir, plumb_expand(e, r->arg, nil));
...
verbset(int obj, Plumbmsg *m, Rule *r, Exec *e)
{
	...
	default:
		...
		break;
	case OData:
		new = plumb_estrdup(plumb_expand(e, r->arg, nil));
		m->ndata = strlen(new);
		free(m->data);
		m->data = new;
		e->p0 = -1;
...
verbadd(int obj, Plumbmsg *m, Rule *r, Exec *e)
{
	...
	default:
		...
		break;
	case OAttr:
		m->attr = plumbaddattr(m->attr, plumbunpackattr(plumb_expand(e, r->arg, nil)));
		return 1;
	}
	return 0;
}
...
newexec(Plumbmsg *m)
{
	...
	exec = plumb_emalloc(sizeof(Exec));
	exec->msg = m;
	exec->p0 = -1;
	exec->p1 = -1;
	return exec;
...
rewrite(Plumbmsg *m, Exec *e)
{
	...
	if(e->clearclick){
		...
		if(e->setdata){
			free(m->data);
			m->data = plumb_estrdup(plumb_expand(e, "$0", nil));
...
buildargv(char *s, Exec *e)
{
	...
	av = nil;
	for(;;){
		av = plumb_erealloc(av, (ac+1) * sizeof(char*));
		av[ac] = nil;
		while(*s==' ' || *s=='\t')
			s++;
		if(*s == '\0')
			break;
		av[ac++] = plumb_estrdup(plumb_expand(e, s, &s));
	}
	return av;
}

Exec*
plumb_matchruleset(Plumbmsg *m, Ruleset *rs)
{
	int i;
	Exec *exec;
...
char*
plumb_startup(Ruleset *rs, Exec *e)
{
	char **argv;
	int i;

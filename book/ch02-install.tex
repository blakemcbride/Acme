...
The Windows build produces a single standalone \file{acme.exe} that
runs on any Windows~10 or later machine with no other files
required --- only Windows system DLLs.  MSYS2 is needed at
\emph{build} time for the toolchain and static libraries; it is
\emph{not} needed at runtime.

\subsection{Prerequisites}

\begin{enumerate}
\item Install MSYS2 from \url{https://www.msys2.org/}.
\item Open the \textbf{MSYS2 UCRT64} shell from the Start menu
      (the icon labeled ``MSYS2 UCRT64'').  Avoid the plain
      ``MSYS2 MSYS'' shell --- it uses the Cygwin-derived MSYS gcc,
      which is not what this build targets.  MINGW64 and CLANG64
      shells also work; the build auto-detects the active subsystem.
...
The \pkg{toolchain} group already pulls in \pkg{pkgconf}, which
provides the \cmd{pkg-config} binary.  Do not install
\pkg{mingw-w64-ucrt-x86\_64-pkg-config} explicitly --- it conflicts
with \pkg{pkgconf}.  Note that the SDL3 package name is lowercase
(\pkg{sdl3}), unlike SDL2.

\paragraph{A note on \texttt{PATH}.}
Several common Windows installs (Strawberry Perl, Anaconda, Git for
Windows) ship their own \cmd{gcc} and \cmd{pkg-config} and place them
on \texttt{PATH} ahead of MSYS2.  Symptoms range from confusing
\cmd{pkg-config} errors (``\texttt{Can't locate Pod/Usage.pm}'') to a
silent missing-header failure during compilation.  If
\cmd{which gcc pkg-config} doesn't resolve under your active
subsystem, prepend it before building:
